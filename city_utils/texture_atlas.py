...
class TextureAtlas():
    ...
    def from_texture_array(textures):
        ...
        for tex in textures:
            sumX += tex.width
            sumY += tex.height
            maxY = max(maxY, tex.height)
        x = 2 ** math.ceil(math.log2(sumX))
        y = 2 ** math.ceil(math.log2(maxY))

        try:
            atlas = TextureAtlas(Rectangle(0, 0, x, y))
            for i, t in enumerate(textures):
                node = atlas.root.insert(t)
                if node is None:
                    print("woops, bad size")
                    raise Exception("Bad size")
                node.image = (i, t)
        except Exception:
            y = 2 ** math.ceil(math.log2(sumY))
            atlas = TextureAtlas(Rectangle(0, 0, x, y))
            for i, t in enumerate(textures):
                node = atlas.root.insert(t)
                node.image = (i, t)
    ...
    def makeTexture(self):
        ...
        sizes = [None for i in range(self.nTextures)]
        translations = [None for i in range(self.nTextures)]
        self.transforms = [None for i in range(self.nTextures)]
        ...
        def walkTree(node):
            ...
            translations[node.image[0]] = (node.rectangle.left, node.rectangle.bottom)
        ...
        for i in range(0, self.nTextures):
            ...
            translation = (translations[i][0] / self.texture.width, translations[i][1] / self.texture.height)
            self.transforms[i] = (translation, scale)
